...
1. **Contract:** run `npm run --silent cli -- author` — the authoring guide for the installed
...
2. **Block specs:** pick the blocks this proposal needs, then run
   `npm run --silent cli -- author --block <type>,<type>` for their fields, examples and schema.
   Fetch only the ones you will use; never author a block from memory.
3. **Write** `proposal.json`, then `npm run --silent cli -- validate proposal.json` — it names
...
4. **Review:** run `npm run --silent cli -- review proposal.json` — serves the UI, blocks
...
6. **Iterate:** run `npm run --silent cli -- iterate proposal.json` — archives the round,
