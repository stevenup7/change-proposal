...
- `npm run cli -- author` — print the versioned authoring guide + block catalog (what the skill fetches). `--block <type>[,<type>]` prints one block's spec (guide + schema fragment); `--schema` the full JSON Schema; `--full` everything inline.
